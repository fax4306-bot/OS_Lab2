...
// 字节序转换函数
// DTB 标准规定，其内部所有多字节数据都使用大端序 (Big-Endian) 存储（高位字节在前，像人 đọc 书写数字的顺序）。而我们实验用的 RISC-V 处理器通常使用小端序 (Little-Endian)（低位字节在前）。
//  DTB 中读出一个 32 位数后，必须通过这个函数将其字节顺序颠倒过来，CPU 才能正确理解它的值。
